...
class Buyer(db_conn.DBConn):
    ...
    def new_order(
        ...
        try:
            ...
            self.new_order_collection.insert_one({
                "order_id": uid,
                "store_id": store_id,
                "user_id": user_id,
    ...
    def payment(self, user_id: str, password: str, order_id: str) -> (int, str):
        try:
            order = self.new_order_collection.find_one({"order_id": order_id})
            if not order:
            ...
            row = self.user_collection.count_documents(condition)
            if row == 0:
                return error.error_not_sufficient_funds(order_id)
            # 先找到符合条件的再改，不然变了
            self.user_collection.update_many(condition, {"$inc": {"balance": -total_price}})
            ...
            row = self.new_order_collection.count_documents({"order_id": order_id})
            self.new_order_collection.delete_many({"order_id": order_id})

            if row == 0:
                return error.error_invalid_order_id(order_id)
            
            row = self.new_order_detail_collection.count_documents({"order_id": order_id})
            self.new_order_detail_collection.delete_many({"order_id": order_id})
            if row == 0:
                return error.error_invalid_order_id(order_id)
            new_order_paid = {
                "order_id": order_id,
            ...
            self.new_order_paid_collection.insert_one(new_order_paid)
    ...
    def add_funds(self, user_id, password, add_value) -> (int, str):
        try:
            result = self.user_collection.find_one({"user_id": user_id})
            row = self.user_collection.count_documents({"user_id": user_id})
            if row == 0:
                return error.error_authorization_fail()
            ...
            self.user_collection.update_one(condition, {"$inc": {"balance": +add_value}})
